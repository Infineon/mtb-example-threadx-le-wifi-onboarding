...
static uint64_t wifi_task_stack[WIFI_TASK_STACK_SIZE/8];
static uint64_t scan_notify_task_stack[NOTIFY_TASK_STACK_SIZE/8];

/* Thread variables*/
cy_thread_t wifi_task_pointer;
cy_thread_t scan_notify_task_pointer;
                                         ...
                                         uint16_t *p_error_handle)
{
    ...
    while (WICED_TRUE)
    {
        *p_error_handle = attr_handle;
        ...
        {
             printf("found type but no attribute found\n");
            free(p_rsp);
            return WICED_BT_GATT_INVALID_HANDLE;
        }
    ...
    wiced_bt_gatt_server_send_read_by_type_rsp(conn_id, opcode, pair_len,
            used, p_rsp, (wiced_bt_gatt_app_context_t)free);
                                                   ...
                                                   uint16_t *p_error_handle)
{
    ...
    *p_error_handle = p_data->handle;
    ...
    case HDLC_CUSTOM_SERVICE_WIFI_PASSWORD_VALUE:
        /* First copy the value to the GATT DB variable and then to the global
         * variable to be stored in NV memory, to be supported in next release.
         */
    ...
    case HDLC_CUSTOM_SERVICE_WIFI_CONTROL_VALUE:
        ...
        printf("Control command %d \n", app_custom_service_wifi_control[0]);
        /* Connect command */
        ...
        {
            ulNotifiedValue = NOTIF_DISCONNECT;
            result = cy_rtos_thread_set_notification(&wifi_task_pointer);
            if (result != CY_RSLT_SUCCESS)
            {
                printf("set thread notification failed \r\n");
        ...
        uint16_t *p_error_handle)
{
    ...
    case GATT_CMD_SIGNED_WRITE:
        ...
        {
            result = wiced_bt_gatt_server_send_write_rsp(p_data->conn_id, p_data->opcode,
                                          ...
                                          wiced_bt_gatt_event_data_t *p_data)
{
    ...
    case GATT_ATTRIBUTE_REQUEST_EVT:
        ...
        {
            result = wiced_bt_gatt_server_send_error_rsp(p_data->attribute_request.conn_id,
                   p_data->attribute_request.opcode,
                                                      error_handle,
                                                      result);
        }
        break;
 ...
 *  and passes a notification to the WiFi task to disconnect from WiFi
 *
 *
 * Parameters:
